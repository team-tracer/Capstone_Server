...
io.sockets.on('connection', (socket) => {
  ...
  socket.on("request_full_duplex",(data)=>{
    let acceptID=data.acceptID; // 알람을 수신한 유저
    let requestID=data.requestID; // 알람을 송신한 유저
    toFrom[requestID]=acceptID;
    userModel.findOne({"id":requestID},(err,user_data)=>{
      let obj={
        "pos_x":user_data.pos_x;
        "pos_y":user_data.pos_y;
      };
